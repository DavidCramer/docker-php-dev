...
LABEL Maintainer="Rheinard Korf <rheinard+docker@thekorfs.com>" \
      Description="PHP-FPM 7.2, xDebug, Composer, PHPUnit, WP-CLI"
...
RUN set -ex \
    ...
    && docker-php-ext-install -j$(nproc) mysqli pdo pgsql pdo_mysql pdo_pgsql intl zip gd \
